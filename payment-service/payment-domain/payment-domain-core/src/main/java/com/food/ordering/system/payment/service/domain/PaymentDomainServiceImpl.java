...
public class PaymentDomainServiceImpl implements PaymentDomainService {
    ...
    public PaymentEvent validateAndInitiatePayment(Payment payment,
        ...
        if (failureMessages.isEmpty()) {
            ...
            return new PaymentCompletedEvent(payment, ZonedDateTime.now(ZoneId.of(UTC_TIMEZONE)), paymentCompletedEventDomainEventPublisher);
        } else {
            log.info("Payment initiation is failed for order id: {}", payment.getOrderId().getValue());
            payment.updateStatus(PaymentStatus.FAILED);
            return new PaymentFailedEvent(payment, ZonedDateTime.now(ZoneId.of(UTC_TIMEZONE)), failureMessages, paymentFailedEventDomainEventPublisher);
        }
    }
    ...
    private void validateCreditHistory(CreditEntry creditEntry, List<CreditHistory> creditHistories, List<String> failureMessages) {
        Money totalCreditHistory = totalCreditHistoryAmount(creditHistories, TransactionType.CREDIT);

        Money totalDebitHistory = totalCreditHistoryAmount(creditHistories, TransactionType.DEBIT);
    ...
    private Money totalCreditHistoryAmount(List<CreditHistory> creditHistories, TransactionType transactionType) {
    ...
    public PaymentEvent validateAndCancelPayment(Payment payment,
        ...
        if (failureMessages.isEmpty()) {
            ...
            return new PaymentCancelledEvent(payment, ZonedDateTime.now(ZoneId.of(UTC_TIMEZONE)), paymentCancelledEventDomainEventPublisher);
        } else {
